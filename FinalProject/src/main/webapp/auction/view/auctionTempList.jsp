...
<script type="text/javascript">
	$(function() {
		...
		if(${spaceuservo.id != null} && ${spaceuservo.id != 'admin'}){
			alert("일반 사용자의 기능이 아닙니다.");
...
<div class="w3-light-gray">
	<div class="container" style="margin-top: 20px;">
		<div class="container">
			<div class="col-lg-12">
				<h2 class="section-heading" align="center">사용자 경매 신청 목록</h2>
				<hr>
				<br>
			</div>
		</div>
				...
				<table class="w3-table w3-bordered" style="background-color: #e6e6e6; width: 80%; margin: auto;">
						<thead>
							<tr style="background-color: #f2d636;">
								...
								<th class="w3-col s2" style="color: white; text-align: center;">신청자</th>
								<th class="w3-col s4" style="color: white; text-align: center;">제목</th>
								...
								<th class="w3-col s1" style="color: white; text-align: center;">삭제</th>
							</tr>
						</thead>
						<tbody>
							<c:forEach items="${listVO.list}" var="auctionTemp">
								<tr class="w3-hover-white">
									...
									<td class="w3-col s2" style="text-align: center;">${auctionTemp.id}</td>
									<td class="w3-col s4"><a
										href="getAuctiontemp.do?a_no=${auctionTemp.a_no}">${auctionTemp.title}</a>
									</td>
									...
									<td class="w3-col s1"
										...
										<div align="center">
											<a href="deleteAuctionTemp.do?a_no=${auctionTemp.a_no}"
												class="btn btn-warning" style="">삭제</a>
										</div>
									</td>
								</tr>
							</c:forEach>
					...
					<br>
					<input type="button" value="신청글 전체삭제" class="btn btn-danger w3-right" style="margin-right: 10%;"
						onclick="javascript:location.href='${initParam.root}deleteAllAuctionTemp.do'">
			</div>
		</div>
		<br>
		<!-- 페이징 처리 -->   
		<%-- 이전 페이지 그룹이 있으면 이미지 보여준다.
			...
			<c:if test="${requestScope.listVO.pagingBean.previousPageGroup}">
				<a href= "getAllAuctionTemp.do?pageNo=${requestScope.listVO.pagingBean.startPageOfPageGroup-1}">
			...
			<c:forEach var="i" begin="${requestScope.listVO.pagingBean.startPageOfPageGroup}" 
				...
				<c:choose>
					<c:when test="${requestScope.listVO.pagingBean.nowPage!=i}">
						<a href="getAllAuctionTemp.do?pageNo=${i}">${i}</a>
					</c:when>
					<c:otherwise>
						${i}
					</c:otherwise>
			   ...
			   <%-- 다음 페이지 그룹이 있으면 화살표 이미지를 보여준다.
			    ...
			    <c:if test="${requestScope.listVO.pagingBean.nextPageGroup}">
			    	<a href= "getAllAuctionTemp.do?pageNo=${requestScope.listVO.pagingBean.endPageOfPageGroup+1}">
